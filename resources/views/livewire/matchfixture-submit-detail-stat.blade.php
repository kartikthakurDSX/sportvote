<div wire:poll.3s>
    ...
    @if (Auth::check())
        @if (in_array(Auth::user()->id, $admins) || Auth::user()->id == $match_fixture->competition->user_id)
            ...
            @if (@$fixture_lapse_type->lapse_type == 4)
                @if ($match_fixture->finishdate_time == null)
                    <!--button class="btn btn-green" data-bs-target="#recordStats" data-bs-toggle="modal"><img src="{{ asset('frontend/images') }}/noun_stats.png" alt="" class="btn-icon">Submit detailed Stats</button-->

                    <!-- <button class="btn btn-submit" data-bs-target="#exampleModal" data-bs-toggle="modal" style="border-radius:.25rem; padding: 0.375rem 0.75rem;"> <img src="{{ asset('frontend/images') }}/noun_finish.png" alt="" class="btn-icon"> Finish Match</button> -->
