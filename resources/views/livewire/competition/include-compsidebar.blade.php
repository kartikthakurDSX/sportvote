<div class="col-md-4" wire:poll.750ms>
    <style>
        .read-more-target,
        ...
        @if (Auth::check())
            ...
            <div class="modal fade" id="addcomprules" tabindex="-1" aria-labelledby="exampleModalLabel"
                ...
                <div class="modal-dialog modal-lg comp_add_rule_model">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title addRule_modelTitle" id="exampleModalLabel">Add a rule for your team
                                admins, players, and referees.</h5>
                                            ...
                                            <div class=" col-md-12 FlotingLabelUp">
                                                <div class="floating-form ">
                                                    <div class="floating-label ">
                                                        ...
                                                        <label class="TeamDescrForm">Add a rule for your team admins,
                                                            players, and referees.</label>
                                                    </div>
                                                </div>
                                                @error('rule_desc')
    ...
    <hr />
    <div class="">
        <span><img src="{{ url('frontend/images/Contact-us.png') }}"></span>
        <span class="AboutStyleUs"> contact US</span>
        @if (Auth::check())
            @if (in_array(Auth::user()->id, $admins) || Auth::user()->id == $competition->user_id)
    ...
    <hr />
    <div class="">
        <span><img src="{{ asset('frontend/images') }}/AdminStar-icon.png"></span>
        <span class="AboutStyleUs"> ADMINISTRATION</span>
        @if (Auth::check())
            @if (in_array(Auth::user()->id, $admins) || Auth::user()->id == $competition->user_id)
                <span> <a style="cursor:pointer;" class="open_admin_popup"><span class="fa-plus"> </span></a>
                </span>
            @else
            @endif
        @else
                                        ...
                                        @if ($comp_mem->invitation_status == 1 && $comp_mem->member->id != Auth::user()->id)
                                            @if ($comp_mem->is_active == 1)
                                                <a class="btn btn-crossSocial crossSocial_a"
                                                    ...
                                                    style="cursor:pointer;"><i class="fas fa-toggle-on"></i></a>
                                            @else
                                                @if ($comp_mem->is_active == 3)
                                                    <a class="btn btn-crossSocial crossSocial_a"
                                                        wire:click ="active_refree({{ $comp_mem->id }})"
                                                        style="cursor:pointer;"><i class="fas fa-toggle-off"></i></a>
                                                @endif
                                            @endif
                                        @else
                                        @endif
    ...
    <hr />
    <div class="">
        <span><img src="{{ url('frontend/images/Desi-Refree-icon.png') }}"></span>
        <span class="AboutStyleUs">DESIGNATED REFEREES</span>
        @if (Auth::check())
            @if (in_array(Auth::user()->id, $admins) || Auth::user()->id == $competition->user_id)
                                    ...
                                    <div class="row">
                                        <div class="mb-4  col-md-12 FlotingLabelUp">
                                            <div class="floating-form ">
                                                <div class="floating-label form-select-fancy-1">
                                                    <input class="floating-input" type="file" id="image"
                                                        placeholder=" " wire:model="sponsor_image">
                                                    <span class="highlight"></span>
                                                    <label>Select Sponsor Image</label>
                                                </div>
                                                @error('sponsor_image')
    ...
    <div class="">
        @if (Auth::check())
            <span><img src="{{ url('frontend/images/Youtube-icon.png') }}"></span> <span class="AboutStyleUs">News &
                Media</span>
            @if (in_array(Auth::user()->id, $admins) || Auth::user()->id == $competition->user_id)
                @if (!empty($video))
                                            ...
                                            <div class="floating-label form-select-fancy-1">
                                                <table>
                                                    ...
                                                    @foreach ($youtubevideo as $comp_video)
                                                        <tr>
                                                            <td>{{ $loop->iteration }}</td>
                                                            <td><a href="{{ $comp_video->video_link }}"> Watch </a>
                                                            </td>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn " style="background-color:#003b5f; color:#fff;"
                                wire:click="editcomp_video">Save changes</button>
                        </div>
                    </div>
                </div>
            </div>
