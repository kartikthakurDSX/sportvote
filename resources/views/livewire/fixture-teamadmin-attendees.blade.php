<div class="row mt-3 mtopNew" {{ $match_fixture->fixture_type == 0 ? 'wire:poll.visible.5s' : '' }}>
        ...
        <div class="col-md-5 col-6 position-relative">
            @if ($fixture_squad_teamOne->isEmpty())
                ...
                @if (in_array(Auth::user()->id, $teamOne_admins_ids))
                    <div class="overlay-button"><button class="btn btn-green modal_teamOne"
                            style="background-color:{{ $teamOne->team_color }};" data-bs-toggle="modal"
                            data-bs-target="#squadModal_teamOne" data-id="{{ $teamOne->id }}">Select Starting
                            Lineup</button></div>
                @else
                    <div class="overlay-button"><button class="btn btn-green"
                            style="background-color:{{ $teamOne->team_color }};" data-bs-toggle="modal">Waiting for
                            players selections</button></div>
                @endif
            @else
                @foreach ($fixture_squad_teamOne as $tm1)
                    <?php $goal1 = App\Models\Match_fixture_stat::where('match_fixture_id', $match_fixture->id)
                        ...
                        ->get(); ?>
                    <?php $red_card = App\Models\Match_fixture_stat::where('match_fixture_id', $match_fixture->id)
                        ->where('team_id', $teamOne->id)
                        ->where('player_id', $tm1->player->id)
                        ->where('sport_stats_id', 3)
                        ->get(); ?>
                    <?php $yellow_card = App\Models\Match_fixture_stat::where('match_fixture_id', $match_fixture->id)
                        ->where('team_id', $teamOne->id)
                        ->where('player_id', $tm1->player->id)
    ...
    <div class="jersy-img-wrap mb-2">
        ...
        <div class="jersy-img">
            <img class="img-fluid" src="{{ url('frontend/images/football.png') }}" alt="" height="100">
        </div>
    </div>
    <div class="jersy-plyr-title d-flex">
...
<div class="col-md-2 Border-RightLeft2">
    ...
    @if ($match_fixture->refree_id != '')
        ...
        <a href="{{ url('player_profile/' . $refree->id) }}" target="a_blank">
            <div class="player-refer-bottom text-center">
                ...
                <h3 class="ref-player" style="bottom:0px">Ref: {{ $refree->first_name }} {{ $refree->last_name }}
                </h3>
    ...
    <div class="mt-4 text-center">
        <!-- <button class="btn btn-green " data-bs-toggle="modal" data-bs-target="#myModal">Select Refree</button> -->
    </div>
</div>
...
<div class="col-md-5 col-6 position-relative BorderMobil">
    @if ($fixture_squad_teamTwo->isEmpty())
        ...
        @if (in_array(Auth::user()->id, $teamTwo_admins_ids))
            <div class="overlay-button"><button class="btn btn-green"
                    style="background-color:{{ $teamTwo->team_color }}; " data-bs-toggle="modal"
                    data-bs-target="#squadModal_teamTwo" data-id="{{ $teamTwo->id }}">Select Starting
                    Lineup</button></div>
        @else
            <div class="overlay-button"><button class="btn btn-green"
                    style="background-color:{{ $teamTwo->team_color }};" data-bs-toggle="modal">Waiting for players
