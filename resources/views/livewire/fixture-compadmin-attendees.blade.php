  ...
  @if ($match_fixture->finishdate_time == "")
      <div class="col-md-5 col-6 position-relative">
          @if ($fixture_squad_teamOne->isEmpty())
              ...
              <div class="overlay-button">
                  <button class="btn btn-green modal_teamOne" style="background-color:{{ $teamOne->team_color }}; color:{{ $teamOne->font_color }};"
                      data-bs-toggle="modal" data-bs-target="#squadModal_teamOne" data-id="{{ $teamOne->id }}">Select Starting Lineup</button>
              </div>
          @else
              @foreach ($fixture_squad_teamOne as $tm1)
                  <?php $goal1 = App\Models\Match_fixture_stat::select('id')->where('match_fixture_id', $match_fixture->id)
                      ->where('team_id', $teamOne->id)
                      ->where('player_id', $tm1->player->id)
                      ->where('sport_stats_id', 1)
                      ->get(); ?>
                  <?php $red_card = App\Models\Match_fixture_stat::select('id')->where('match_fixture_id', $match_fixture->id)
                      ->where('team_id', $teamOne->id)
                      ->where('player_id', $tm1->player->id)
                      ->where('sport_stats_id', 3)
                      ->get(); ?>
                  <?php $yellow_card = App\Models\Match_fixture_stat::select('id')->where('match_fixture_id', $match_fixture->id)
                      ->where('team_id', $teamOne->id)
                      ->where('player_id', $tm1->player->id)
                      ->where('sport_stats_id', 2)
                      ->get(); ?>
                  <?php $team_member = App\Models\Team_member::select('jersey_number')->where('member_id', $tm1->player_id)
                      ->where('team_id', $teamOne->id)
                      ->first(); ?>
                  ...
                  <div class="jersy-img-wrap mb-2">
                      ...
                      <div class="jersy-img">
                          <img class="img-fluid"
                              src="{{ url('frontend/profile_pic') }}/{{ $tm1->player->profile_pic }}" alt=""
                              height="100">
                      </div>
      ...
      @endforeach
          ...
          @else
              <div class="player-jersy-list">
          @endif
          <?php $ownGoalCount = App\Models\Match_fixture_stat::select('id')->where('match_fixture_id', $match_fixture->id)
                  ->where('team_id', $teamOne->id)
                  ->where('player_id', 16)
  ...
  <div class="row mb-4 p-3">
      <div class="col-md-12 col-12 ">
          ...
          @foreach ($subplyr1 as $tm1)
              <?php
              $team_member = App\Models\Team_member::select('jersey_number')->where('member_id', $tm1->player_id)
                  ->where('team_id', $teamOne->id)
                  ->first();

              $goal1 = App\Models\Match_fixture_stat::select('id')->where('match_fixture_id', $match_fixture->id)
                  ->where('team_id', $teamOne->id)
                  ->where('player_id', $tm1->player->id)
                  ->where('sport_stats_id', 1)
                  ->get();
...
<div class="col-md-2 Border-RightLeft2">
  ...
  <div class="mt-4 text-center">
      @if (count($comp_referees) > 0 && $match_fixture->startdate_time == null)
          <button class="btn btn-green " data-bs-toggle="modal" data-bs-target="#select_refree">Select Referee</button>
      @else
      @endif
  </div>
</div>
...
<div class="col-md-5 col-6 position-relative BorderMobil">
  @if ($fixture_squad_teamTwo->isEmpty())
      ...
      <div class="overlay-button"><button class="btn btn-green"
              ...
              data-bs-toggle="modal" data-bs-target="#squadModal_teamTwo" data-id="{{ $teamTwo->id }}">Select
  ...
  @else
      @foreach ($fixture_squad_teamTwo as $tm1)
          ...
          <div class="jersy-img-wrap mb-2">
              ...
              <div class="jersy-img">
                  <img class="img-fluid" src="{{ url('frontend/profile_pic') }}/{{ $tm1->player->profile_pic }}"
                      alt="" height="100">
              </div>
  ...
  @endforeach
  <?php $ownGoalCount2 = App\Models\Match_fixture_stat::select('id')->where('match_fixture_id', $match_fixture->id)
                  ->where('team_id', $teamTwo->id)
                  ->where('player_id', 16)
                  ->where('sport_stats_id', 54)
                  ->get(); ?>
...
<div class="row mb-4 p-3">
  ...
  <div class="col-md-12 col-12 ">
      ...
      @foreach ($subplyr2 as $tm2)
          ...
          ->where('sport_stats_id', 1)
          ->get();
          $team_member = App\Models\Team_member::select('jersey_number')->where('member_id', $tm2->player_id)
              ->where('team_id', $teamTwo->id)
              ->first(); ?>
          <div class="player-jersy-list player_score" data-id="{{ $tm2->player_id }},{{ $teamTwo->id }}">
              <div class="jersy-img-wrap mb-2">
                  <style>
                      .jersy2::after {
                          color: <?php echo $teamTwo->team_color; ?>;
                  ...
                  <div class="jersy-img">
                      <img class="img-fluid"
                          src="{{ url('frontend/profile_pic') }}/{{ $tm2->player->profile_pic }}" height="100">
                  </div>
...
@endif
  ...
  <div class="modal fade" id="select_refree" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" wire:ignore.self>
                  ...
                  <select class="select grey-form-control input-sm" wire:model="selectedReferee_id" >
                      @if ($comp_referees->isNotEmpty())
                          ...
                          @foreach ($comp_referees as $refree)
                              <option value="{{ $refree->member->id }}" @if($refree->member->id == $match_fixture->refree_id) selected @else @endif>{{ $refree->member->first_name }} {{ $refree->member->last_name }}</option>
                          @endforeach
                      @else
                      @endif
                  </select>
