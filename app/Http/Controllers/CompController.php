...
class CompController extends Controller
{
    public function index()
    {
        ...
        $competition_request = Competition_team_request::where('user_id', $user_id)->with('competition','team')->get();
        // return $my_comp;
	...
	public function participate_in()
	{
		...
		// return $user_participateComp_data;
	...
	public function all_competitions()
	{
		...
		$all_comp = Competition::where('user_id','!=',$user_id)->with('sport','sport_comp','comptype','compsubtype','comp_fixture')->orderby('id', 'DESC')->get();
	...
	public function my_competitions()
	{
		if(Auth::check())
        {
            $user_id = Auth::user()->id;
        ...
        $my_comp = Competition::where('user_id', $user_id)->where('comp_type_id','!=', NULL)->where('is_active','!=', 2)->with('sport','sport_comp','comptype','compsubtype','comp_fixture')->orderby('id', 'DESC')->get();
		$my_draft_comp = Competition::where('user_id', $user_id)->where('is_active', 0)->with('sport')->latest()->get();
    ...
    public function create()
    {
        ...
        return view('frontend.competitions.create',compact('teams','team_stats','com_type','sport_level','player_stat','team_stat','comp_admin','user','current_step','com_report_type','competition_logo','referees','member_position','sport_level','player_stat','admin','team_ranking_stats'));
    }
    ...
    {
		session(['createComp_id' => ""]);
		$competition = Competition::with('user')->where('is_active','!=', 2)->find($id);
		...
		if($competition->comp_season_start != "" && $competition->comp_season_end != ""){
			$cal_comp_season_start = date('M',strtotime($competition->comp_season_start)). " '". date('y',strtotime($competition->comp_season_start));
			$cal_comp_season_end = date('M',strtotime($competition->comp_season_end)). " '". date('y',strtotime($competition->comp_season_end));
		...
		$fixture_ids = array();
			...
			foreach($match_fixtures as $match_fixture)
			{
				...
				if($match_fixture->finishdate_time != null){
					...
					if($cal_current_time >= $voting_finish_time)
					{
						...
						$Playervoting = voting::groupBy('player_id')->where('match_fixture_id', $match_fixture->id)->selectRaw('count(*) as total, player_id')->get();
						...
						if($Playervoting->count() > 0)
						{
							...
							$mvpPlayer_team_id = voting::where('match_fixture_id',$match_fixture->id)->where('player_id', $mvp_player_id)->where('status_desc', 0)->first();
							$check_mvp_record = voting::where('match_fixture_id',$match_fixture->id)->where('status_desc', 1)->get();
			...
			if($competition->comp_type_id == 2)
			{
				...
				$latest_match_fixtures = Match_fixture::select('fixture_round')->where('competition_id',$competition->id)->latest()->first();
				//return $latest_match_fixtures;

				// kO FIXTURE TABLE
				$valid_round = ['4','8','16','32','64','128'];
				$first_tab_teams = Competition_team_request::select('team_id')->where('competition_id',$id)->where('request_status',1)->with('team')->get();
				$match_fixtures = Match_fixture::where('competition_id',$id)->get();
				$fixtureteamoneids = array();
				$fixtureteamtwoids = array();
				foreach($match_fixtures as $fixtureteam){
					$fixtureteamoneids[] = $fixtureteam['teamOne_id'];
					$fixtureteamtwoids[] = $fixtureteam['teamTwo_id'];
				}
				$fixtureteamids = array_merge($fixtureteamoneids, $fixtureteamtwoids);
				...
				return view('frontend.competitions.ko_competition',compact('competition','comp_season','total_fixture','admins','matches_played','total_goals','sports_stat','top_team_goal','top_player_goal','recent_winner','avg_goal','valid_round','first_tab_teams','match_fixtures','fixtureteamids','latest_match_fixtures','fixture_ids','team_stat'));
			}
			elseif($competition->comp_type_id == 3)
			{
				...
				$l_fixtures_date = Match_fixture::where('competition_id',$competition->id)->latest()->first();
				...
				{
					$firstL_won = Match_fixture::where('competition_id',$competition->id)->where('winner_team_id',$first_fixtures->teamOne->id)->count();
					$firstR_won = Match_fixture::where('competition_id',$competition->id)->where('winner_team_id',$first_fixtures->teamTwo->id)->count();
					$firstL_team_rank = Competition_team_request::select('rank')->where('competition_id',$competition->id)->where('team_id',$first_fixtures->teamOne->id)->first();
					$firstR_team_rank = Competition_team_request::select('rank')->where('competition_id',$competition->id)->where('team_id',$first_fixtures->teamTwo->id)->first();
					...
					$firstL_played = Match_fixture::where(function ($query) use ($fL_team_id) {
						...
						})->where('competition_id',$competition->id)->where('finishdate_time','!=',Null)->orderBy('finishdate_time', 'DESC')->get();
					
					$fR_team_id = $first_fixtures->teamTwo->id;
					$firstL_draw = Match_fixture::where(function ($query) use ($fL_team_id) {
						$query->where('teamOne_id', '=', $fL_team_id)
						->orWhere('teamTwo_id', '=', $fL_team_id);
						})->where('competition_id',$competition->id)->where('fixture_type',1)->count();

					$firstL_lost = Match_fixture::where(function ($query) use ($fL_team_id) {
						$query->where('teamOne_id', '=', $fL_team_id)
						->orWhere('teamTwo_id', '=', $fL_team_id);
						})->where('competition_id',$competition->id)->where('winner_team_id','!=',$fL_team_id)->count();
					...
					$firstR_lost = Match_fixture::where(function ($query) use ($fR_team_id) {
						$query->where('teamOne_id', '=', $fR_team_id)
						->orWhere('teamTwo_id', '=', $fR_team_id);
						})->where('competition_id',$competition->id)->where('winner_team_id','!=',$fR_team_id)->count();
					$firstR_win_points = $firstR_won * 3;
					$firstR_draw_points = $firstR_draw *1;
					$firstR_team_points = $firstR_win_points + $firstR_draw_points;
					$firstL_yellowcards = Match_fixture_stat::where('competition_id',$competition->id)->where('team_id',$fL_team_id)->where('sport_stats_id',2)->count();
					$firstR_yellowcards = Match_fixture_stat::where('competition_id',$competition->id)->where('team_id',$fR_team_id)->where('sport_stats_id',2)->count();
					$firstL_redcards = Match_fixture_stat::where('competition_id',$competition->id)->where('team_id',$fL_team_id)->where('sport_stats_id',3)->count();
					$firstR_redcards = Match_fixture_stat::where('competition_id',$competition->id)->where('team_id',$fR_team_id)->where('sport_stats_id',3)->count();
					$firstL_goal_for = Match_fixture_stat::where('competition_id',$competition->id)->where('team_id',$fL_team_id)->whereIn('sport_stats_id',[1,54])->count();
					$firstR_goal_for = Match_fixture_stat::where('competition_id',$competition->id)->where('team_id',$fR_team_id)->whereIn('sport_stats_id',[1,54])->count();
				...
				// dd($againts_team);
					...
					return view('frontend.competitions.leage_competition',compact('competition','comp_season','total_rounds','team_type','round_fixtures','comp_teams','comp_fixtures','admins','comp_referee','month_array','lastest_fixture_year','l_fixtures_date','accepted_comp_team','team_stat','player_stat','recent_winner','fixture_ids','first_fixtures','firstL_team_points','firstR_team_points','firstL_played','firstR_played','firstL_yellowcards','firstR_yellowcards','firstL_redcards','firstR_redcards','firstL_won','firstR_won','firstL_draw','firstL_lost','firstR_draw','firstR_lost','firstL_goal_for','firstR_goal_for','firstR_againts_goals','firstR_goal_differ','firstL_againts_goals','firstL_goal_differ','comp_teamOne_ids','comp_teamTwo_ids','team_ids','total_goals','avg_goal','firstL_team_rank','firstR_team_rank'));
				}
				return view('frontend.competitions.leage_competition',compact('competition','comp_season','total_rounds','team_type','round_fixtures','comp_teams','comp_fixtures','admins','comp_referee','month_array','lastest_fixture_year','l_fixtures_date','accepted_comp_team','team_stat','player_stat','recent_winner','fixture_ids','first_fixtures','total_goals','avg_goal'));
				...
				// return $competition;
				return view('frontend.competitions.view',compact('match_fixture','competition_teams','competition','team_member','user','accepted_teams','admins'));
			}
    }
    ...
    {
		session(['createComp_id' => ""]);
		$competition = Competition::with(['user' => function ($query) { $query->select('id','first_name','last_name','email','profile_pic','created_at','dob');
                        }])->where('is_active','!=', 2)->find($id);
		$user_profile = User_profile::where('user_id', Auth::user()->id)->where('profile_type_id',3)->first();
		$comp_sponsers = Sponsor::where('module_type',1)->where('type',2)->where('type_id',$competition->id)->where('is_active', 1)->get();
		if($user_profile)
		{
			$comp_memeber = Comp_member::where(['comp_id' => $competition->id , 'member_id' => Auth::user()->id, 'member_position_id' => 6])->whereIn('invitation_status',[0,1])->first();
		}
		else
		{
			$comp_memeber = "";
		...
		$com_report_type = Comp_report_type::where('sport_id',1)->get();
		$is_follow = User_fav_follow::where('is_type',2)->where('type_id',$competition->id)->where('user_id',Auth::user()->id)->first();
		if(!empty($is_follow))
		{
			if($is_follow->Is_follow == 1)
			{
				$is_follow = 1;
			}
			else
		...
		$comp_follower = User_fav_follow::where('is_type',2)->where('type_id',$competition->id)->where('Is_follow',1)->where('is_active',1)->get();
		$match_fixtures = Match_fixture::select('id','finishdate_time','teamOne_id','teamTwo_id')->where('competition_id', $competition->id)->get();
		if($competition->comp_season_start != "" && $competition->comp_season_end != ""){
			$cal_comp_season_start = date('M',strtotime($competition->comp_season_start)). " '". date('y',strtotime($competition->comp_season_start));
			$cal_comp_season_end = date('M',strtotime($competition->comp_season_end)). " '". date('y',strtotime($competition->comp_season_end));
			$comp_season = $cal_comp_season_start." - ".$cal_comp_season_end;
			...
			{
				$fixture_ids[] = $match_fixture->id;
                $fixtureteamoneids[] = $match_fixture['teamOne_id'];
				$fixtureteamtwoids[] = $match_fixture['teamTwo_id'];
				...
				if($match_fixture->finishdate_time != null){
					...
					if($cal_current_time >= $voting_finish_time)
					{
						...
						if($Playervoting->count() > 0)
						{
							...
							$mvpPlayer_team_id = voting::select('team_id')->where('match_fixture_id',$match_fixture->id)->where('player_id', $mvp_player_id)->where('status_desc', 0)->first();
							$check_mvp_record = voting::select('id')->where('match_fixture_id',$match_fixture->id)->where('status_desc', 1)->get();
							if(count($check_mvp_record) == 0){
			...
			{
				//Basic stats for team and player
				$sports_stat = Sport_stat::select('id','name')->whereIn('stat_type_id',[0,1])->where('is_active',1)->where('stat_type',1)->get();
				$team_stat = Sport_stat::select('id','name')->whereIN('stat_type_id',[0,2])->whereIn('stat_type',[0,1])->where('is_active',1)->get();
				$player_stat = Sport_stat::select('id','name')->whereIN('stat_type_id',[0,1])->whereIn('stat_type',[0,1])->where('is_active',1)->get();
			}
			else
			{
				//Detailed stats for team and player
				...
				$team_stat = Sport_stat::select('id','name')->whereIn('id', $stats_array)->where('is_active',1)->get();
				$sports_stat = Sport_stat::select('id','name')->whereIn('stat_type_id', [1,54])->where('is_active',1)->get();
				$player_stat =  Sport_stat::select('id','name')->whereIn('id',$stats_array)->where('is_active',1)->get();
			}
			$mvp_winner_list = Fixture_squad::groupBy('player_id')->whereIn('match_fixture_id', $fixture_ids)->where('status_desc',1)->selectRaw('count(*) as total, player_id')->get();
			// $recent_winner = $mvp_winner_list->toArray();
			...
			$default_search = $current_year."-".str_pad($current_month,2,'0',STR_PAD_LEFT);
			$check_fixtures = Match_fixture::where('competition_id',$competition->id)->where('fixture_date', 'like', '%'.$default_search.'%')->with('teamOne:id,name,team_logo','teamTwo:id,name,team_logo')->orderBy('fixture_date', 'ASC')->get();
			// return $competition_fixture_list;
			$comp_teams = Competition_team_request::select('team_id')->where('competition_id',$id)->where('request_status',1)->with('team:id,name,team_logo')->get();
			$comp_fixtures = Match_fixture::select('competition_id','teamOne_id','teamTwo_id')->where('competition_id',$competition->id)->with('competition:id,name','teamOne:id,name,team_logo','teamTwo:id,name,team_logo')->get();
			$comp_teamOne = array();
			$comp_teamTwo = array();
			foreach($comp_fixtures as $fixture)
			{
			...
			$comp_admins = Comp_member::where('comp_id',$competition->id)->where('member_position_id',7)->where('invitation_status',1)->where('is_active', 1)->with('member')->pluck('member_id');
			$admins = $comp_admins->toArray();
			$total_goals = Match_fixture_stat::select('id')->whereIn('match_fixture_id',$fixture_ids)->whereIn('stat_type',[1,4])->where('is_active',1)->whereIn('sport_stats_id',[1,54])->count();
			$matches_played = Match_fixture::select('id')->where('competition_id',$competition->id)->where('startdate_time','!=',NULL)->where('finishdate_time','!=',NULL)->count();
				if($matches_played > 0)
				{
					$avg_goal = $total_goals / $matches_played;
				}
				else
				{
					$avg_goal = 0;
			...
			if($competition->comp_type_id == 2)
			{
				$total_fixture = Match_fixture::select('id')->where('competition_id', $competition->id)->where('fixture_type','!=', 9)->count();
				$matches_played = Match_fixture::select('id')->where('competition_id',$competition->id)->where('startdate_time','!=',NULL)->where('finishdate_time','!=',NULL)->count();
				$latest_match_fixtures = Match_fixture::select('fixture_round')->where('competition_id',$competition->id)->latest()->first();
				...
				$first_sports_stat = Sport_stat::select('id')->whereIn('stat_type_id',[0,1,2])->where('calc_type',1)->where('is_active',1)->first();
				$team_goals = Match_fixture_stat::where('competition_id',$competition->id)->where('sport_stats_id',$first_sports_stat->id)->get();
				$top_team_goal = $team_goals->groupBy('team_id');
				$top_player_goal = $team_goals->groupBy('player_id');
				$winner_ids = array('NULL',0);
			...
			elseif($competition->comp_type_id == 3)
			{
				...
				$comp_referee = Comp_member::select('member_id')->where('comp_id',$competition->id)->where('member_position_id',6)->where('invitation_status',1)->where('is_active',1)->count();
				$month_array = array("JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC");
				$l_fixtures_date = Match_fixture::select('fixture_date')->where('competition_id',$competition->id)->latest()->first();
				if(!empty($l_fixtures_date))
				{
					$lastest_fixture_year = date('Y', strtotime($l_fixtures_date->fixture_date));
				}
				...
				$accepted_comp_team = Competition_team_request::select('id')->where('competition_id',$competition->id)->where('request_status',1)->count();
				//Quick compare graph
				$first_fixtures = Match_fixture::select('id','competition_id','teamOne_id','teamTwo_id')->where('competition_id',$competition->id)->where('finishdate_time','!=',null)->with('competition:id,name','teamOne:id,name,team_logo,team_color','teamTwo:id,name,team_logo,team_color')->first();
				if(!empty($first_fixtures))
				{
					$firstL_won = Match_fixture::select('id')->where('competition_id',$competition->id)->where('winner_team_id',$first_fixtures->teamOne->id)->count();
					$firstR_won = Match_fixture::select('id')->where('competition_id',$competition->id)->where('winner_team_id',$first_fixtures->teamTwo->id)->count();
					$firstL_team_rank = Competition_team_request::select('rank')->where('competition_id',$competition->id)->where('team_id',$first_fixtures->teamOne->id)->first();
					$firstR_team_rank = Competition_team_request::select('rank')->where('competition_id',$competition->id)->where('team_id',$first_fixtures->teamTwo->id)->first();

					$fL_team_id = $first_fixtures->teamOne->id;
					$firstL_played = Match_fixture::select('winner_team_id')->where(function ($query) use ($fL_team_id) {
						$query->where('teamOne_id', '=', $fL_team_id)
						->orWhere('teamTwo_id', '=', $fL_team_id);
						})->where('competition_id',$competition->id)->where('finishdate_time','!=',Null)->orderBy('finishdate_time', 'DESC')->get();
					
					$fR_team_id = $first_fixtures->teamTwo->id;
					$firstL_draw = Match_fixture::select('id')->where(function ($query) use ($fL_team_id) {
						$query->where('teamOne_id', '=', $fL_team_id)
						->orWhere('teamTwo_id', '=', $fL_team_id);
						})->where('competition_id',$competition->id)->where('fixture_type',1)->count();

					$firstL_lost = Match_fixture::select('id')->where(function ($query) use ($fL_team_id) {
						$query->where('teamOne_id', '=', $fL_team_id)
						->orWhere('teamTwo_id', '=', $fL_team_id);
						})->where('competition_id',$competition->id)->where('winner_team_id','!=',$fL_team_id)->count();
					$firstL_win_points = $firstL_won * 3;
					...
					$firstR_played = Match_fixture::select('winner_team_id')->where(function ($query) use ($fR_team_id) {
						$query->where('teamOne_id', '=', $fR_team_id)
						->orWhere('teamTwo_id', '=', $fR_team_id);
						})->where('competition_id',$competition->id)->where('finishdate_time','!=',Null)->orderBy('finishdate_time', 'DESC')->get();
					
					$firstR_draw = Match_fixture::select('id')->where(function ($query) use ($fR_team_id) {
						$query->where('teamOne_id', '=', $fR_team_id)
						->orWhere('teamTwo_id', '=', $fR_team_id);
						})->where('competition_id',$competition->id)->where('fixture_type',1)->count();
					$firstR_lost = Match_fixture::select('id')->where(function ($query) use ($fR_team_id) {
						$query->where('teamOne_id', '=', $fR_team_id)
						->orWhere('teamTwo_id', '=', $fR_team_id);
						})->where('competition_id',$competition->id)->where('winner_team_id','!=',$fR_team_id)->count();
					$firstR_win_points = $firstR_won * 3;
					$firstR_draw_points = $firstR_draw *1;
					$firstR_team_points = $firstR_win_points + $firstR_draw_points;
					$firstL_yellowcards = Match_fixture_stat::select('id')->where('competition_id',$competition->id)->where('team_id',$fL_team_id)->where('sport_stats_id',2)->count();
					$firstR_yellowcards = Match_fixture_stat::select('id')->where('competition_id',$competition->id)->where('team_id',$fR_team_id)->where('sport_stats_id',2)->count();
					$firstL_redcards = Match_fixture_stat::select('id')->where('competition_id',$competition->id)->where('team_id',$fL_team_id)->where('sport_stats_id',3)->count();
					$firstR_redcards = Match_fixture_stat::select('id')->where('competition_id',$competition->id)->where('team_id',$fR_team_id)->where('sport_stats_id',3)->count();
					$firstL_goal_for = Match_fixture_stat::select('id')->where('competition_id',$competition->id)->where('team_id',$fL_team_id)->whereIn('sport_stats_id',[1,54])->count();
					$firstR_goal_for = Match_fixture_stat::select('id')->where('competition_id',$competition->id)->where('team_id',$fR_team_id)->whereIn('sport_stats_id',[1,54])->count();
				...
				// dd($againts_team);
					foreach($againts_firstL_team as $a_team)
					{
						if($a_team->teamOne_id == $fL_team_id)
						{
							$goal_a = Match_fixture_stat::select('id')->where('competition_id',$competition->id)->where('team_id',$a_team->teamTwo_id)->where('match_fixture_id',$a_team->id)->whereIn('sport_stats_id',[1,54])->where('is_active',1)->count();
							$firstL_againts_goals = $firstL_againts_goals + $goal_a;
						}
						else
						{
							$goal_a = Match_fixture_stat::select('id')->where('competition_id',$competition->id)->where('team_id',$a_team->teamOne_id)->where('match_fixture_id',$a_team->id)->whereIn('sport_stats_id',[1,54])->where('is_active',1)->count();
							$firstL_againts_goals = $firstL_againts_goals + $goal_a;
						}
					}
					$firstL_goal_differ = $firstL_goal_for - $firstL_againts_goals;
					...
					return view('frontend.competitions.leagueComp',compact('competition','user_profile','comp_memeber','comp_sponsers','is_follow','com_report_type','comp_follower','comp_season','total_rounds','team_type','round_fixtures','comp_teams','comp_fixtures','admins','comp_referee','check_fixtures','month_array','lastest_fixture_year','l_fixtures_date','accepted_comp_team','team_stat','player_stat','recent_winner','fixture_ids','first_fixtures','firstL_team_points',
								'firstR_team_points','firstL_played','firstR_played','firstL_yellowcards','firstR_yellowcards','firstL_redcards','firstR_redcards','firstL_won','firstR_won','firstL_draw','firstL_lost','firstR_draw','firstR_lost','firstL_goal_for','firstR_goal_for','firstR_againts_goals','firstR_goal_differ','firstL_againts_goals','firstL_goal_differ','comp_teamOne_ids','comp_teamTwo_ids','team_ids','total_goals','avg_goal','firstL_team_rank','firstR_team_rank'));
				}
				return view('frontend.competitions.leagueComp',compact('competition','user_profile','comp_memeber','comp_sponsers','is_follow','com_report_type','comp_follower','comp_season','total_rounds','team_type','round_fixtures','comp_teams','comp_fixtures','admins','comp_referee','check_fixtures','month_array','lastest_fixture_year','l_fixtures_date','accepted_comp_team','team_stat','player_stat','recent_winner','fixture_ids','first_fixtures','total_goals','avg_goal'));
			}
			elseif($competition->comp_type_id == 1) {
	...
	public function comp_info(Request $request)
	{
		$compid = $request->compeId;
		$comp_info = Competition::select('id','name','location','report_type','vote_mins','squad_players_num','lineup_players_num','comp_type_id','comp_season_start','comp_season_end')->find($compid);
	...
	{
		$edit_comp = Competition::find($request->compeId);
		if($request->lineup_players_num > 0 && $request->lineup_players_num > 0)
		{
			if($request->lineup_players_num <= $request->squad_players_num)
			{
				$edit_comp->name = $request->comp_name;
				$edit_comp->location = $request->location;
				$edit_comp->report_type = $request->report_type;
				$edit_comp->vote_mins = $request->vote_mins;
				$edit_comp->squad_players_num = $request->squad_players_num;
				$edit_comp->lineup_players_num = $request->lineup_players_num;
				if($request->comp_type_id != 1)
				{
					if($request->seasonStart != "" && $request->seasonEnd != ""){
						$edit_comp->comp_season_start = date('Y-m-01', strtotime($request->seasonStart));
						$edit_comp->comp_season_end = date('Y-m-01', strtotime($request->seasonEnd));
					}
				}
				$edit_comp->save();
				if($edit_comp){
					return response()->json(["status"=>1,'comp_info' => $edit_comp]);
				}
			}
			else
			{
				return response()->json(["status"=>0,'message'=>'Starting Players should be equal or less than Squad players']);
			}
		}
		else
		{

			return response()->json(["status"=>0, 'message'=>'Please enter squad Or lineup players number']);
		}
	}
   ...
   // KO Competition Page
		...
		for ($x = 0; $x < count($request->admins_ids); $x++)
		{
			...
			if(!empty($check_comp_member))
			{
				...
				else
				{
					...
					$notifications = Notification::where('notify_module_id', 4)->where('type_id', $update_request->id)->
									where('reciver_id',  $request->admins_ids[$x])->get();
			...
			else
			{
				if($request->admins_ids[$x] != Auth::user()->id)
				{
					...
					$notifications = Notification::where('notify_module_id', 4)->where('type_id', $send_request->id)->
										where('reciver_id',  $request->admins_ids[$x])->get();
					if(count($notifications) == 0){
						$notification = Notification::create([
						'notify_module_id' => 4,
			...
			foreach($data as $player_id => $player_goal)
			{
				...
				<div class=" W-100">
					<li class="list-group-item d-flex justify-content-between align-items-start">
						...
						<img class="img-fluid rounded-circle rounded-circle padd-RL" src="<?php echo url('frontend/profile_pic')?>/<?php echo $player_info->profile_pic; ?>" style="width:25% !important;">
						<div class="ms-2 me-auto EngCity">
							<div class=" ManCity">
	...
	public function top_teams_sats_list(Request $request)
	{
		...
		?>
			...
			<div class=" W-100">
				<li class="list-group-item d-flex justify-content-between align-items-start">
					<img class="img-fluid rounded-circle rounded-circle padd-RL" src="<?php echo url('frontend/logo')?>/<?php echo $team_info->team_logo; ?>" style="width:25% !important;">
					<div class="ms-2 me-auto EngCity">
	...
	public function league_quick_compare(Request $request)
	{
                    ...
                    <div class="row">
                        <div class="col-md-4 col-12 W-50tab p-12">
                            <div class="RankedSec">
                        		...
                        		<p class="RankedNo"><?php if($team_fixtures->count() > 0){ echo str_pad($left_team_rank->rank, 2, 0,STR_PAD_LEFT); } else { echo "##"; } ?></p>
                    		</div>
                		</div>
                        <div class="col-md-4 col-12 W-50tab p-12">
                            <div class=" borderPoint">
                                ...
                                <p class="RankedNo"><?php if($team_fixtures->count() > 0){ echo str_pad($left_team_points, 2, 0,STR_PAD_LEFT); } else { echo "##"; } ?> </p>
                            </div>
                        </div>
        				...
        				<div class="col-md-4 col-12 TabView ">
            				<div class=" borderPoint">
                                ...
                                <p class="line-height-Team-form">
									<?php
									$rtf = 0;
									foreach($left_team_played as $a_team){ $rtf++;
									if($rtf <= 5){
        				...
        				<div class="col-md-4 col-12 pl-0 tabFull">
            				<div class="d-flex justi_centr-tab">
                                ...
                                <div class="cards-det">
                                    ...
                                    <p class="RedD mb-2"><?php echo str_pad($left_team_againts_goals, 2, 0,STR_PAD_LEFT);  ?></p>
                                </div>
            				</div>
        				</div>
    				</div>
                    ...
                    <div class="row">
                        <div class="col-md-4 col-12 W-50tab p-12">
                            <div class="RankedSec">
                                ...
                                <p class="RankedNo"><?php if($right_team_played->count() > 0) {echo str_pad($right_team_rank->rank, 2, 0,STR_PAD_LEFT);} else { echo "##"; } ?></p>
                            </div>
                        </div>
                        <div class="col-md-4 col-12 W-50tab  p-12">
                            <div class=" borderPoint">
                                ...
                                <p class="RankedNo"><?php if($right_team_played->count() > 0) { echo str_pad($right_team_points, 2, 0,STR_PAD_LEFT);} else { echo "##"; }  ?></p>
                            </div>
                        </div>
                        <div class="col-md-4 col-12 New-on-Tab p-12">
                            <div class="d-flex">
        				...
        				<div class="col-md-4 col-12 TabView">
            				<div class=" borderPoint">
                                ...
                                <p class="line-height-Team-form">
                                   <?php
									$Rrtf = 0;
									foreach($right_team_played as $a_team){ $Rrtf++;
									if($Rrtf <= 5){
										if($a_team->fixture_type == 1){
            			...
            			<div class="col-md-4 col-12  pl-0 tabFull">
               				<div class="d-flex justi_centr-tab ">
                                ...
                                <div class="cards-det">
                                    ...
                                    <p class="RedD mb-2"><?php echo str_pad($right_team_againts_goals, 2, 0,STR_PAD_LEFT);  ?></p>
                                </div>
                			</div>
            			</div>
        			</div>
		...
		{
			?> <div class="col-md-12 col-12 pr-0 "> <p class="text-center"> No Data Found</p> <?php
		}
				...
				for($winners = 0; $winners < count($vote_count_key); $winners++)
				{
					if($i < 5)
					{
						...
						?>
							...
							<img class="img-fluid rounded-circle rounded-circle padd-RL" src="<?php echo url('frontend/profile_pic') ?>/<?php echo $mvpplayer->profile_pic; ?>" width="25%">
							<div class="ms-2 me-auto EngCity">
   ...
   public function public_top_performer(Request $request)
   {
		...
		for($tp = 0 ; $tp < $counter; $tp++)
		{
			...
			<div class="top-performer-box w-100 d-flex" >
				...
				<div class="performer-detail  col-md-5 py-2" style="background:<?php echo $team->team_color; ?> !important;">
   ...
   public function get_fixtures(Request $request)
	{
		$comp_id = $request->comp_id;
		$fistureMonth = $request->fistureMonth;
		$fixtureYear = $request->fixtureYear;
		$search_date = $fixtureYear."-".str_pad($fistureMonth,2,'0',STR_PAD_LEFT);
		$check_fixtures = Match_fixture::where('competition_id',$comp_id)->where('fixture_date', 'like', '%'.$search_date.'%')->with('teamOne:id,name,team_logo','teamTwo:id,name,team_logo')->orderBy('fixture_date', 'ASC')->get();
		$competition = Competition::select('comp_type_id','comp_start')->find($comp_id);
		$c_fixtures = array();
		if(count($check_fixtures) > 0){
			...
			$fixHtml = "";
			$fixHtml.= '<div class="owlfixtureCal owl-carousel owl-theme">';
			foreach($fixture_chunks as $chunks)
			{
				$fixHtml .= '<div class="item">
							<table class="table TableFixtureCalndr ">';
						foreach($chunks as $fixture){
							$Comp_type = competition_type::select('id','name')->find($competition->comp_type_id);
							...
							$fixHtml .= '<tr><td>';
								$matchlink = $url."/match-fixture/".$fixture->id;
								$team1Link = $url."/team/".$fixture->teamOne_id;
								$team1Name = substr($fixture->teamOne->name,0,13);
								$team1Img = $url."/frontend/logo/".$fixture->teamOne->team_logo;
								$team2Link = $url."/team/".$fixture->teamTwo_id;
								$team2Name = substr($fixture->teamTwo->name,0,13);
								$team2Img = $url."/frontend/logo/".$fixture->teamTwo->team_logo;
								$fixtureDay = date("D", strtotime($fixture->fixture_date));
								$fixtureDate = date("M d", strtotime($fixture->fixture_date));
								if($competition->comp_start == 1)
								{
									$fixHtml .= '<a href="'.$matchlink.'" target="_blank"> <span class="OnSun">'.$fixtureDay.'</span> <span class="Dec-DateFix">'.$fixtureDate.'</span>
									</a>';
								}else{
									$fixHtml .= '<span class="comp_start0" data-toggle="tooltip" data-placement="bottom" data-original-title="" title="Competition not start yet!"> <span class="OnSun">'.$fixtureDay.'</span> <span class="Dec-DateFix">'.$fixtureDate.'</span> </span>';
								}
								$fixHtml .= '</td><td class="RightPosiText ">';
								$fixHtml .= '<a href="'.$team1Link.'" target="_blank" data-toggle="tooltip" data-placement="bottom" title="'.$fixture->teamOne->name.'" data-original-title=""> <b class="WolVerWand">'.$team1Name.'</b> </a>&nbsp;
									<div class="pp-pageHW"><a href="'.$team1Link.'" target="_blank" data-toggle="tooltip" data-placement="bottom" title="'.$fixture->teamOne->name.'" data-original-title=""> <img class="img-fluid rounded-circle" src="'.$team1Img.'"></a>
									</div></td><td class="BtnCentr">';
								if($fixture->startdate_time == NULL){
									if($competition->comp_start == 1){
										$fixHtml .= '<a href="'.$matchlink.'" class="btn btn-gray text-center btn-xs-nb" target="_blank" data-toggle="tooltip" data-placement="bottom" title="Competition Name: '.$fixture->competition->name.', Competition type: '.$Comp_type->name.', TeamOne: '.$fixture->teamOne->name.', TeamTwo: '.$fixture->teamTwo->name.'">'. date("H:i", strtotime($fixture->fixture_date)).'</a>';
									}else{
										$fixHtml .= '<a title="Competition not start yet!" class="btn btn-gray text-center btn-xs-nb comp_start0"  target="_blank" data-toggle="tooltip" data-placement="bottom" title="Competition Name: '.$fixture->competition->name.', Competition type: '.$Comp_type->name.', TeamOne: '.$fixture->teamOne->name.', TeamTwo: '.$fixture->teamTwo->name.'">'.date("H:i", strtotime($fixture->fixture_date)).'</a>';
									}
								}else{
									$teamOneGoal = Match_fixture_stat::select('id')->where('match_fixture_id',$fixture->id)->where('team_id',$fixture->teamOne_id)->whereIn('sport_stats_id',[1,54])->count();
									$teamTwoGoal = Match_fixture_stat::select('id')->where('match_fixture_id',$fixture->id)->where('team_id',$fixture->teamTwo_id)->whereIn('sport_stats_id',[1,54])->count();
							
									$fixHtml .= '<a href="'.$matchlink.'" target="_blank" data-toggle="tooltip" data-placement="bottom" title="Competition Name: '.$competition->name.', Competition Type: '.$Comp_type->name.', TeamOne: '.$fixture->teamOne->name.', TeamTwo: '.$fixture->teamTwo->name.'">
										<span class=" btn-greenFXL " target="_blank">'.str_pad($teamOneGoal, 2, 0,STR_PAD_LEFT).'</span>
										<span class=" btn-greenFXR ">'.str_pad($teamTwoGoal, 2, 0,STR_PAD_LEFT).'</span> </a>';
								}
								$fixHtml .= '</td><td class="LeftPosiText ">
									<div class="pp-pageHW"><a href="'.$team2Link.'" target="_blank" data-toggle="tooltip" data-placement="bottom" title="'.$fixture->teamTwo->name.'" data-original-title=""> <img class="img-fluid " src="'.$team2Img.'"></a></div>&nbsp;
									<a href="'.$team2Link.'" target="_blank" data-toggle="tooltip" data-placement="bottom" title="'.$fixture->teamTwo->name.'" data-original-title=""> <b class="WolVerWand">'.$team2Name.'</b></a>
									</td><td><span class="row teamFixutreView"><span class="col-md-2">';
									if($fixture->startdate_time == NULL){
										$fixHtml .= '<span class="IconPLR"><a onclick="return ics_file('.$fixture->id.');"  style="cursor:pointer;"><i class="fa-solid fa-calendar-plus"></i></a></span>';
									}else{
										if($fixture->finishdate_time != NULL){
											if($fixture->fixture_type == 1){
												$fixHtml .= '<div class="Drawmatch" data-toggle="tooltip" data-placement="bottom" data-original-title="" title="Match draw"></div>';
											}else{
												if($fixture->winner_team_id != NULL){
													$winner_team_logo = Team::select('id','team_logo','name')->find($fixture->winner_team_id);
													$winerTeamUrl = $url."/team/".$winner_team_logo->id;
													$winerTeamLogo = $url."/frontend/logo/".$winner_team_logo->team_logo;
													$fixHtml .= '<span class="tableViewFixture pp-pageHW ">
														<a href="'.$winerTeamUrl.'" target="_blank" data-toggle="tooltip" data-placement="bottom" data-original-title="" title="Match won">
														<img class="rounded-circle img-fluid" src="'.$winerTeamLogo.'" height="30px">
														</a>
													</span>';
												}else{
													$fixHtml .= '<div class="VotingOpenClor"></div>';
												}
											}
										}else{
											$fixHtml .= '<div class="VotingOpenClor"></div>';
									...
									$fixHtml .= '</span><span class="col-md-10">';
									if($fixture->startdate_time == NULL){
										$fixHtml .= '<a class="text-decoration AndyMcg" onclick="return ics_file('.$fixture->id.');" style="cursor:pointer"> Add to calendar <i class="fa-solid fa-angles-right"></i></a>';
									}else{
										$fixHtml .= '<a class="text-decoration AndyMcg" href="'.$matchlink.'" target="_blank"> VIEW FIXTURE <i class="fa-solid fa-angles-right"></i></a>';
									}
									$fixHtml .= '</span></span></td></tr>';
						}
						$fixHtml .= '</table></div>';
			}
			$fixHtml .= '</div>';
		}else{
			$fixHtml = '<p class="text-center"> No Data Found!!</p>';
		}
		return $fixHtml;
	}
